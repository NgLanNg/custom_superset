...
class ScenarioWritebackView(BaseSupersetView):
    ...
    def writeback(self) -> Any:
        ...
        try:
            logger.info(f"Writeback payload: {payload}")
            value = float(payload.get("value", 0))
        except (ValueError, TypeError):
            logger.error(f"Invalid value in payload: {payload.get('value')}")
        ...
        logger.info(f"Writeback for user: {user_email}")
        ...
        try:
            ...
            return jsonify({"status": "error", "message": f"Database write failed: {str(e)}"}), 500
