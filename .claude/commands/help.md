...
1. **Context Loading**:
   - Always read `vault/active_context.md` and `vault/tasks/todo_{brief}_{date}.md` first.
...
4. **Discipline Injection**:
   ...
   - Inject **Discovery** and **Verification** gates into the `vault/tasks/todo_{brief}_{date}.md`.
...
| :--- | :--- | :--- | :--- |
| **P1** | Analysis | `init`, `scan`, `brainstorm`, `audit`, `mcp`, `likeaboss` | Understand, discover, design |
| **P2** | Planning | `spec`, `todo` | Define scope, break down tasks |
| **P3** | Solutioning | `tickets` | Epics/stories, architecture |
| **P4** | Implementation | `sprint`, `review`, `docs`, `clean`, `rethink` | Build, verify, deploy |
...
| **Project Path** | New products, major features, services | High | `/init` -> `/brainstorm` -> `/spec` -> `/tickets` -> `/sprint` |
...
```txt
/init -> /brainstorm -> /spec -> /tickets -> /sprint
  P1       P1          P2       P3         P4
```
...
| 2 | `/brainstorm` | Design decisions in `docs/plans/` | No design = unclear direction = wasted effort |
| 3 | `/spec` | Technical spec in `docs/specs/` | No spec = unclear requirements = wrong implementation |
| 4 | `/tickets` | Epics/Stories in `docs/specs/epics.md` | No tickets = no traceability = scope creep |
| 5 | `/sprint` | Sprint plan | Ready for implementation |
...
| 3 | `/todo` | Task list in `vault/tasks/todo_{brief}_{date}.md` | No tasks = no progress tracking = blocked sprints |
