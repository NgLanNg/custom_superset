...
osascript -e 'tell application "Terminal" to do script "cd '$PWD' && export PYTHONPATH=$PWD && cd superset && export FLASK_ENV=development && export FLASK_APP=superset && export SUPERSET_SECRET_KEY=TEST && ./.venv/bin/superset run -p 8088 --with-threads --reload --debugger"'
