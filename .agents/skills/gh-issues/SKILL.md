...
For each confirmed issue, dispatch a sub-agent to implement the fix. Launch up to 8 concurrently. Use context optimization flags where available to mitigate rate limits.
...
For each issue, construct the following prompt and pass it to the sub-agent dispatcher. Variables to inject into the template:
