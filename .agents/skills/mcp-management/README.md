...
```bash
cd .agents/skills/mcp-management/scripts
npm install
```
...
See `.agents/.mcp.json.example` for more examples.
...
**Note**: The CLI automatically looks for config at `~/.agents/.mcp.json`. You can run it from any directory.
...
- Load config from `.agents/.mcp.json`
...
2. `.agents/skills/mcp-management/.env`
3. `.agents/skills/.env`
4. `.agents/.env`
...
The `mcp-manager` agent (`.agents/agents/mcp-manager.md`) uses this skill to:
...
Ensure `.agents/.mcp.json` exists and is valid JSON.
