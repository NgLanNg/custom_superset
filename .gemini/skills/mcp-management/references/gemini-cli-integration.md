...
Gemini CLI reads MCP servers from `.gemini/settings.json`. Create a symlink to `.agents/.mcp.json`:
...
ln -sf .agents/.mcp.json .gemini/settings.json

# Create symlink (Windows - requires admin or developer mode)
mklink .gemini\settings.json .agents\.mcp.json
```
...
1. **Configuration Loading**: Reads `.gemini/settings.json` (symlinked to `.agents/.mcp.json`)
...
Should show symlink pointing to `.agents/.mcp.json`.
