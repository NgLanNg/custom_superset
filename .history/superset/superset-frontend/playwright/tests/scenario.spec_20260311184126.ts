...
test.describe('Scenario Plugin E2E', () => {
    ...
    test('should render Scenario chart and handle write-back', async ({ page }) => {
        ...
        // 4. Switch to Growth tab to find growth projects
        await page.getByText('Growth Configuration').click();

        // 5. Verify data row presence (LNGC2 is a growth project)
        await expect(page.getByText('LNGC2')).toBeVisible({ timeout: 20000 });

        // 6. Perform write-back on the first input in the LNGC2 row
