...
|-------|---------------|----------|
| Local | `.agents.json` (in project) | Personal/experimental |
| Project | `.mcp.json` (project root) | Team-shared |
| User | `~/.agents.json` | Cross-project personal |
...
# Check user config
cat ~/.agents.json 2>/dev/null | grep -E '"(aws-mcp|aws|awsdocs)"'
...
1. **IAM Role** (for EC2/ECS/Lambda): No additional config needed - uses instance credentials
