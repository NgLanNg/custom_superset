...
# In a real Superset env, we would use @protect(). For local UX test, we can leave it open or use it.
...
def writeback() -> Any:
    ...
    user_email = current_user.email if hasattr(current_user, "email") else "anonymous@local"
    ...
    try:
        ...
        db.session.rollback()
        logger.error(f"Failed to upsert scenario data: {e}", exc_info=True)
...
def health() -> Any:
    return jsonify({"status": "ok", "blueprint": "scenario", "db": "sqlite"}), 200
