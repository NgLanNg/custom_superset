...
jest.mock('../src/useScenarioData', () => ({
  useScenarioData: jest.fn(() => ({
    existingData: [
      { bu: 'LNGA', opu: 'MLNG', year: 2020, value: 70 },
      { bu: 'LNGA', opu: 'ALNG', year: 2020, value: 60 },
    ],
    growthData: [],
    stats: { totalAssets: 10, avgEquity: 50 },
    chartOptions: {},
    handleSave: jest.fn(),
    saveBatch: jest.fn(),
    isSaving: false,
...
jest.mock('../src/useGrowthConfig', () => ({
  useGrowthConfig: jest.fn(() => ({
    rows: [
      { project: 'LNGC2', fid_year: 2025, capacity_mtpa: 5.0, status: 'On Track' },
    ],
    handleSave: jest.fn(),
    isSaving: false,
...
describe('ScenarioChart', () => {
  ...
  test('renders tables with existing and growth rows', () => {
    render(
      <ThemeProvider theme={supersetTheme}>
        <ScenarioChart {...defaultProps} />
      </ThemeProvider>
  ...
  test('handles cell edit optimistic update', async () => {
    ...
    useScenarioData.mockReturnValue({
      existingData: [{ bu: 'LNGA', opu: 'MLNG', year: 2020, value: 70 }],
      growthData: [],
      stats: { totalAssets: 10, avgEquity: 50 },
      chartOptions: {},
    ...
    await waitFor(() => {
      expect(mockHandleSave).toHaveBeenCalledWith('MLNG', 2020, 55);
    });
  });
