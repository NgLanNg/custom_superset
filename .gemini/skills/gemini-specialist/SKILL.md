...
description: Spawns a dedicated Gemini subagent by dynamically loading a specific YAML role file. Use this when you need massive context windows (entire codebases), multi-modal capabilities, or fast bulk analysis from one of the roles in .agents/roles/sub/.
---
...
```bash
.agents/skills/gemini-specialist/scripts/gemini-runner.sh .agents/roles/sub/data/data_engineer.yaml "Review all python files in the src/pipeline directory and map the data lineage."
```

#### Example 2: UX Designer

```bash
.agents/skills/gemini-specialist/scripts/gemini-runner.sh .agents/roles/sub/analysis/ux_designer.yaml "Analyze the user flow described in docs/onboarding.md and suggest friction reductions."
```
