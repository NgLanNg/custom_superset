...
class Api(BaseSupersetView):
    ...
    @handle_api_exception
    @has_access_api
    @expose("/v1/writeback/", methods=("POST",))
    def writeback(self) -> FlaskResponse:
        """
        Handle writeback operations from frontend custom React plugins (like AG Grid editable table).
        """
        data = request.json
        logger.info(f"Writeback operation invoked with data: {data}")
        # Normally here we would persist the change matching rowId to database record
        # For POC, we log and return success
