  ...
  ({
    ...
    const handleCellValueChanged = useCallback(async (event: CellValueChangedEvent) => {
      if (event.oldValue !== event.newValue) {
        try {
          // Trigger the REST endpoint back to Superset backend. Wait for API to resolve
          // In an actual large-scale deployed feature, slice_id and table details should be passed cleanly
          await SupersetClient.post({
            endpoint: '/api/v1/writeback/', // to be implemented in Python Flask Backend
            jsonPayload: {
              column: event.colDef.field,
              newValue: event.newValue,
              oldValue: event.oldValue,
              rowId: event.data.id || event.rowIndex, // assume id exists or fall back to rowIndex
              rowData: event.data
            }
          });
        } catch (error) {
          console.error('Failed to write back data', error);
          // Optionally revert back to old value
