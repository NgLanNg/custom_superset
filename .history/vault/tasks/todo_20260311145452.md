...
### T29 -- Verify dbt pipeline end-to-end

- **Action:** Run full `dbt build` (run + test). Query the gold table to confirm deduplication works (edit a cell via UI, re-run dbt, confirm only latest value survives)
- **Done when:** `dbt build` passes with 0 errors, gold table row count <= silver table row count, edited values reflected
- **Risk:** Low
- **Status:** [ ]
